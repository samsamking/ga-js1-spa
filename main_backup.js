...
(function() {
	...
	function renderPopup(data, into){
		...
		document.querySelector('#add-button').addEventListener('click', (event) => {
			...
			firebase.database().ref(`tasks/${dataCaption}/`).set({
				total: starRatingTotal + rating
			});
		...
		firebase.database().ref(`tasks/${dataCaption}/`).on('value', function(snapshot) {
			...
			if(state != null){
				...
				Object.keys(state.comments).map((key) => {
					var starRating = parseInt(state.comments[key].stars);
